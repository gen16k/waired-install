...
#   darwin_*      macOS: download tarball + Ollama.app, register LaunchAgent
...
WAIRED_INSTALL_REPO="${WAIRED_INSTALL_REPO:-gen16k/waired-install}"
...
# Linux only: the upstream Ollama install one-liner. linux_apt_install
# pipes it to sh when no ollama is already present (the Linux analogue of
# darwin_install_ollama). Override to pin a version or point at an
# internal mirror.
WAIRED_OLLAMA_LINUX_URL="${WAIRED_OLLAMA_LINUX_URL:-https://ollama.com/install.sh}"
...
Usage:
  curl -fsSL https://github.com/gen16k/waired-install/releases/latest/download/install.sh | sh
  curl -fsSL https://github.com/gen16k/waired-install/releases/latest/download/install.sh | sh -s -- --dev
  curl -fsSL https://github.com/gen16k/waired-install/releases/latest/download/install.sh | sh -s -- --dry-run
...
Options:
  ...
  --edge, --latest install the latest main build (same as
...
Environment variables:
  ...
  WAIRED_OLLAMA_LINUX_URL  Linux only: override the Ollama install script URL
                           (default: https://ollama.com/install.sh)
...
# so auto-init is skipped on re-runs / updates of an enrolled host.
linux_enrolled() {
    [ -e /var/lib/waired/identity.json ]
}
...
# package the host lacks). The .deb postinst preserves /etc/waired and
# restarts the systemd unit, so identity/state survive untouched.
linux_apt_update() {
...
linux_apt_install() {
    ...
    if ollama_skip_requested; then
        ...
        ollama_status="skipped (--skip-ollama / WAIRED_NO_OLLAMA; install later: curl -fsSL $WAIRED_OLLAMA_LINUX_URL | sh)"
    else
...
Quickstart:   https://github.com/gen16k/waired/blob/main/docs/quickstarts/README.md
...
# Install Ollama on Linux via the upstream one-liner. Reuse an existing
# install if `ollama` is already on PATH; otherwise pipe
# https://ollama.com/install.sh to sh — it self-elevates via sudo, drops
# the binary into /usr/local/bin, and registers its own systemd unit.
# This is the Linux analogue of darwin_install_ollama; the bundled model
# itself is pulled later by waired-agent at boot. Set WAIRED_NO_OLLAMA to
# skip. A failure here is non-fatal: waired-agent retries the pull once a
# usable ollama appears.
linux_install_ollama() {
    if command -v ollama >/dev/null 2>&1; then
        common_log "Ollama already present at $(command -v ollama) — skipping install"
        return 0
    fi
    common_log "Installing Ollama from $WAIRED_OLLAMA_LINUX_URL"
    if [ "$DRY_RUN" = 1 ]; then
        common_log "  (dry-run) would: curl -fsSL $WAIRED_OLLAMA_LINUX_URL | sh"
        return 0
    fi
    if ! curl -fsSL "$WAIRED_OLLAMA_LINUX_URL" | sh; then
        common_warn "Ollama install failed; waired-agent will retry pulling the bundled model at boot. Install by hand later: curl -fsSL $WAIRED_OLLAMA_LINUX_URL | sh"
    fi
}
...
# per-user LaunchAgent via `waired-agent install`. curl-downloaded
# binaries do NOT get the Gatekeeper quarantine xattr (only browser /
# LSFileQuarantine downloads do), so unsigned binaries run fine here;
# code signing / notarization is a follow-up (#262).
# ---------------------------------------------------------------------
...
darwin_install() {
    ...
    if [ "$(id -u)" -eq 0 ]; then
        common_warn "running as root: the LaunchAgent and state dir will belong to root, not your login user."
        common_warn "re-run as your normal user (sudo is invoked only for the /usr/local/bin copy) for the intended per-user install."
    fi

    state_dir="$HOME/Library/Application Support/waired"
...
# Register the per-user LaunchAgent. No sudo: the job lives in the
# user's gui/<uid> domain and the state dir is under $HOME.
darwin_register_agent() {
    state_dir="$1"
    common_log "Registering waired-agent LaunchAgent (per-user, no sudo)"
    if [ "$DRY_RUN" = 1 ]; then
        common_log "  (dry-run) would: $WAIRED_DARWIN_BINDIR/waired-agent install --state-dir \"$state_dir\""
        return 0
    fi
    "$WAIRED_DARWIN_BINDIR/waired-agent" install --state-dir "$state_dir"
}

# darwin_maybe_init finishes first-run setup on macOS. The per-user
# LaunchAgent (registered + started just above) reads state under $HOME and
# runs as the login user — so init needs no sudo and there's no state-dir /
# owner split: it signs in via the running daemon, which also asks the
# coding-agent integration consent (default Yes) and applies it for this
# user. Skipped when --no-init, already enrolled, or there is no
# controlling terminal (init's sign-in is interactive).
darwin_maybe_init() {
    state_dir="$1"
    [ "$FLAG_NO_INIT" = 1 ] && return 0
    ...
    if ! tty_available; then
        common_log "$(emo '💡' 'Note:') No terminal detected — run 'waired init' (or use the tray) to sign in."
        return 0
    fi
    if [ "$DRY_RUN" = 1 ]; then
        common_log "  (dry-run) would: waired init </dev/tty"
        return 0
    fi
    common_log "$(emo '🔑' '>>') Starting sign-in (waired init)…"
    "$WAIRED_DARWIN_BINDIR/waired" init </dev/tty || \
        common_warn "sign-in did not complete; finish later with: waired init"
}
...
# darwin_restart_agent reloads the per-user LaunchAgent so the freshly
# swapped binary takes effect, falling back to (re-)registration if the
# job is not currently loaded.
darwin_restart_agent() {
    common_log "Restarting waired-agent (launchctl kickstart)"
    if [ "$DRY_RUN" = 1 ]; then
        common_log "  (dry-run) would: launchctl kickstart -k gui/$(id -u)/com.waired.agent"
        return 0
    fi
    if ! launchctl kickstart -k "gui/$(id -u)/com.waired.agent" 2>/dev/null; then
        common_warn "LaunchAgent not loaded; (re-)registering it."
        darwin_register_agent "$HOME/Library/Application Support/waired"
    fi
}
...
darwin_update() {
    ...
    darwin_maybe_init "$HOME/Library/Application Support/waired"
    common_log "Ollama: managed separately; not modified by update."
...
darwin_next_steps() {
  ...
  1. Sign in: waired init     (or open the tray app → \"Log in…\")
...
$tray_line
State dir:   $state_dir
LaunchAgent: ~/Library/LaunchAgents/com.waired.agent.plist (per-user, starts at login)
Ollama:      $ollama_status

$get_started

The agent runs as a per-user LaunchAgent and starts at each login.
$tray_step
Diagnostics:  waired doctor
              log show --predicate 'process == "waired-agent"' --last 5m
Uninstall:    waired-agent uninstall
...
main() {
    ...
    # --latest) is enough on every OS. Derive the per-OS opt-in the user
    # would otherwise have to know — the edge apt suite (Linux) and the
    # edge prerelease asset base (macOS) — unless they pinned those
    ...
    if [ "$(channel_from_env)" = edge ]; then
        ...
        if [ -z "$_WAIRED_INSTALL_BASE_URL_SET" ]; then
            WAIRED_INSTALL_BASE_URL=https://github.com/gen16k/waired-install/releases/download/edge
        fi
    fi
